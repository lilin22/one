...
from testCases.org.org import org
...
def check(userReady):
    ...
    o = org(one_uri,mobile,password,nickname,message)
    response = o.org_list(token)
    ids = []
    for i in range(len(response['data']['manage_orgs'])):
        if i >= 1:
            ids.append(response['data']['manage_orgs'][i]['id'])
    if ids != []:
        o.org_del(token,ids)
